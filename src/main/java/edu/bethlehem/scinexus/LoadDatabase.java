...
class LoadDatabase {
  ...
  @Bean
  CommandLineRunner initDatabase(AcademicRepository academicRepository, ArticleRepository articleRepository,
      MediaRepository mediaRepository, PostRepository postRepository, OrganizationRepository organizationRepository,
    ...
    return args -> {
      ...
      for (Name name : names) {
        academics.add(new Academic(name.toString(), name.getFirstName(),
            "pass1234" + name.getFirstName(), name.getLastName() + name.getFirstName() +"" +new Random(k).nextInt() + "@bethlehem.edu"));
      k++;
      }
      try {
        ...
        for (int i = 0; i < bound; i++) {
          ...
          articles.add(new Article("title" + academic.getName(), "description" + academic.getEmail(), "subject", academic,
                  "content"));
        }
        ...
        for (int i = 0; i < bound; i++) {
          Academic academic = academics.get(random.nextInt(academics.size()));
          organizations.add(new Organization("name" + academic.getName(), "username" + academic.getName() +""+new Random(i).nextInt(),
      ...
      try {
        ...
        for (int i = 0; i < bound; i++) {
          Academic academic = academics.get(random.nextInt(academics.size()));
          researchPapers
                  .add(new ResearchPaper("Title" + academic.getName(), "Description" + academic.getEmail(), "Subject",
                          academic));
        }
