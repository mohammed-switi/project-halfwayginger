...
//                Long journalId = Long.parseLong(object.getVariables().get("journalId"));
//                User user = (User) authentication.get().getPrincipal();
//                System.out.println(isJournalOwner(journalId,user) + "hello++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
...
//    }
    ...
    public org.springframework.security.authorization.
        ...
        return (authentication, context) -> {
            ...
            try {
                Map<String, Object> requestBody = mapper.readValue(context.getRequest().getInputStream(), Map.class);

                System.out.println("journalOwnerFIRST");
                System.out.println("this is request Body "+requestBody.toString() );
                System.out.println("THIs is the GOal : "+requestBody.get("journalId"));
            int journalId = (int) requestBody.get("journalId");

            Journal journal = journalRepository.findById((long) journalId)
                    .orElseThrow(() -> new JournalNotFoundException(String.valueOf(journalId),
                            HttpStatus.NOT_FOUND));
            User user = (User) authentication.get().getPrincipal();
            // check if the user is the publisher journal

                System.out.println( "this is journalOwnerDecision"+journal.getPublisher().getId().equals(user.getId()));
